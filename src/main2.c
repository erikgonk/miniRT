...
/*   Updated: 2024/11/12 12:19:50 by erigonza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
float sphere_ray_intersect(Vector3 ray_start, Vector3 ray_direction, Vector3 sphere_center, float sphere_radius) {
    Vector3 s = subtract(ray_start, sphere_center);
    float b = dot(s, ray_direction);
    float c = dot(s, s) - sphere_radius * sphere_radius;
    float h = b * b - c;
    if (h < 0.0) {
        return -1.0f; // No intersection
    }
    h = sqrtf(h);
    return fmaxf(-b - h, 0.0f); // Return the closest intersection distance
}
...
Vector3 sphere_normal(Vector3 sphere_center, Vector3 point) {
    Vector3 n = subtract(point, sphere_center);
    float norm = sqrtf(dot(n, n));
    Vector3 result = { n.x / norm, n.y / norm, n.z / norm };
    return result;
}

// Helper function to draw a pixel in the MLX image
    ...
    for (int y = 0; y < WINDOW_HEIGHT; y++) {
        for (int x = 0; x < WINDOW_WIDTH; x++) {
            ...
            if (t > 0.0f) {
                ...
                // Simple shading: use the normal to create a grayscale color
